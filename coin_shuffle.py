...
class Round(object):
    ...
    def protocol_definition(self):
        ...
        inputs = {player:self.__coin.address(self.__players[player])  for player in self.__players}
        # self.__logchan.send(str(inputs))
        # (amount, fee, inputs, outputs, changes):
        transaction = self.__coin.make_transaction(self.__amount, self.__fee, inputs, self.__new_addresses, self.__change_addresses)
