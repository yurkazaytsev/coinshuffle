...
class Commutator(object):
    ...
    def _recv(self):
        ...
        while response[-3:] != self.frame:
            # try:
            # self.debug('recv ' + str(len(response)) + "received")
    ...
    def send(self, message):
        ...
        while request:
            try:
                ...
                self.in_lifo(self._recv())
                self.debug('recv before send')
            except:
                self.debug('send after recv try')
                self.socket.settimeout(self.timeout)
    ...
    def recv(self):
        # r2r, r2w, r2e = select.select([self.socket],[self.socket],[],self.switch_time)
        if self.in_lifo:
            self.debug('recv from buffer: socets state')
            return self.in_lifo.popleft()
        else:
            recieved = False
            while not recieved:
                try:
                    self.debug('recv from socket: socets state ')
                    return self._recv()
                    recieved = True
                except:
                    continue
